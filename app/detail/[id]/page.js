...
export default async function DetailPage({ params }) {
  ...
  // Proteksi data jika response atau video_data null
  const data = response?.data?.video_data;

  if (!data) {
    return (
      <div className="text-center py-20 text-slate-400">
        Data drama tidak ditemukan di bawah salju...
      </div>
    );
  }
  ...
  let categories = [];
  try {
    categories = JSON.parse(data.category_schema || "[]");
  } catch (e) {
    categories = [];
  }
    ...
    <div className="max-w-6xl mx-auto px-6">
      <div className="flex flex-col md:flex-row gap-10 items-start">
        ...
        <div className="w-full md:w-72 shrink-0 shadow-2xl shadow-cyan-500/10 rounded-3xl overflow-hidden border border-white/10">
          <img src={data.series_cover} alt={data.series_title} className="w-full h-auto" />
        </div>
        ...
        <div className="flex-1">
          ...
          <div className="flex flex-wrap gap-2 mb-6">
            {categories.map((cat, idx) => (
              <span key={idx} className="text-xs font-bold px-4 py-1.5 rounded-full bg-cyan-500/10 border border-cyan-500/20 text-cyan-300">
                {cat.name}
              </span>
            ))}
          </div>
